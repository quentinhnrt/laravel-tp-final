@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
            ...
            <div>
                <span class="relative z-0 inline-flex rtl:flex-row-reverse shadow-sm rounded-md gap-x-2">
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                    ...
                    @else
                        <a href="{{ $paginator->previousPageUrl() }}" rel="prev"
                           class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-white bg-theme-500 border border-theme-500 rounded-md leading-5 hover:text-theme-800 focus:z-10 focus:outline-none focus:ring ring-theme-300 focus:border-background-300 active:bg-background-100 active:text-theme-800 transition ease-in-out duration-150"
                    ...
                    @foreach ($elements as $element)
                        ...
                        @if (is_string($element))
                            <span aria-disabled="true">
                                <span
                                    class="relative inline-flex rounded-md items-center px-4 py-2 -ml-px text-sm font-medium text-background-400 dark:text-background-700 bg-white border cursor-default leading-5 dark:bg-background-800 dark:border-background-600">{{ $element }}</span>
                            </span>
                        @endif
